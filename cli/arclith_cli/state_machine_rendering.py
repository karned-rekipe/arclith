...
def render_state_documentation(
    ...
    return (
        ...
        f"Pour une entité créée avec ce profil, le champ `{spec.state_field}` démarre\n"
        f"à `{spec.initial_state}` et `{entity}State` définit son type et ses valeurs\n"
        "persistées. Pour une entité existante, le blueprint conserve sa valeur par\n"
        "défaut ou son caractère requis ainsi que son propre `Literal` ou enum ;\n"
        f"`{entity}State` sert alors uniquement de matrice interne au cycle de vie\n"
        "généré.\n\n"
