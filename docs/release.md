...
tmp_dir="$(mktemp -d)"
cd "$tmp_dir"
uvx --from arclith-cli==0.13.0 arclith-cli init pantry-agent --dir .
cd pantry-agent
uv sync
uv run python -c "import arclith; print(arclith.__version__ if hasattr(arclith, '__version__') else 'arclith import ok')"
uvx --from arclith-cli==0.13.0 arclith-cli capabilities
uvx --from arclith-cli==0.13.0 arclith-cli add-entity ShoppingItem
uvx --from arclith-cli==0.13.0 arclith-cli add-usecase PlanShoppingList
uvx --from arclith-cli==0.13.0 arclith-cli add-intent-interpreter ShoppingIntent
```
